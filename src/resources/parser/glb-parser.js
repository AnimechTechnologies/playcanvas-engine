Object.assign(pc, function () {
    ...
    GlbParser.parseAsync = function (filename, urlBase, data, device, defaultMaterial, assetRegistry, extensionRegistry, callback) {
        // parse the data
        parseChunk(filename, data, function (err, chunks) {
            ...
            parseGltf(chunks.gltfChunk, function (err, gltf) {
                ...
                loadBuffersAsync(gltf, chunks.binaryChunk, urlBase, function (err, buffers) {
                    ...
                    // async load images
                    loadTexturesAsync(device, gltf, buffers, urlBase, assetRegistry, function (err, textures) {
                        if (err) {
                            callback(err);
                            return;
                        }

                        createResources(device, gltf, buffers, textures, defaultMaterial, extensionRegistry, callback);
                    });
                });
            });
        });
            ...
            if (err) {
                ...
                parseGltf(chunks.gltfChunk, function (err, gltf) {
                    if (err) {
                        ...
                        // create resources
                        createResources(device, gltf, buffers, textures, defaultMaterial, extensionRegistry, function (err, result_) {
                            if (err) {
                                console.error(err);
                            } else {
                                result = result_;
